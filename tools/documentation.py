...
def split_into_chunks(text, max_tokens=2048):
    """
    Splits a long text into chunks that fit within the token limit for AI models.
    """
...
def generate_ai_description(ai_client, code, context):
    """
    Generates a description using the local AI model, handling large inputs by chunking.
    """
    debug_print(f"Generating description for the following {context}:\n{code}")

    prompt = f"Generate a detailed and concise description for the following {context}:\n\n{code}"

    # Check if the code exceeds the max length and split if necessary
    chunks = split_into_chunks(prompt)

    try:
        description = ""
        for idx, chunk in enumerate(chunks):
            # Build the history and make the AI call
            debug_print(f"Processing chunk {idx + 1} of {len(chunks)}")
            debug_print(f"Sending to AI:\n{chunk}")  # Print exactly what is being sent to the AI
            ...
            # Debugging the AI response
            debug_print(f"AI Response: {response}")  # Print the full response received from the AI

            # Extract the content from the response
            if response and "choices" in response and len(response.choices) > 0:
                description_chunk = response.choices[0].message.get('content', '')

                if not description_chunk.strip():
                    # If the content is empty, warn the user
                    debug_print(f"Warning: AI response returned empty content for chunk {idx + 1}.")
                    description += "Description not available."
                else:
                    debug_print(f"Extracted description chunk:\n{description_chunk}")
                    description += description_chunk.strip()

            else:
                # If response is missing or invalid
                debug_print(f"Warning: No valid choices found in AI response for chunk {idx + 1}.")
                description += "Description not available."

        return description if description else "Description not available."

    except Exception as e:
        debug_print(f"Error generating description: {e}")
        return "Description not available."


def generate_description(ai_client, code, context):
    """
    Generates a placeholder description using Lorem Ipsum.
    """
    # You can replace this text with any other placeholder content as needed.
    ...
    debug_print(f"Returning placeholder description for {context}.")
    return placeholder_description


def create_rst_file(module_path, output_dir, relative_path, ai_client):
    """
    Creates an RST file for the Python module, adding descriptions generated by AI.
    """
    debug_print(f"Creating RST file for module: {module_path}")
...
"""
    if functions:
        ...
        for func_node in functions:
            ...
            debug_print(f"Generating description for function: {func_name}")
            func_description = generate_description(ai_client, func_code, "function")
    ...
    if classes:
        ...
        for cls_node in classes:
            ...
            debug_print(f"Generating description for class: {cls_name}")
            class_description = generate_description(ai_client, cls_name, "class")
...
"""
            ...
            if methods:
                ...
                for method_node in methods:
                    ...
                    debug_print(f"Generating description for method: {method_name}")
                    method_description = generate_description(ai_client, method_code, "method")
    ...
    with open(os.path.join(output_dir, f"{relative_path}.rst"), "w", encoding="utf-8") as f:
        f.write(rst_content)
    debug_print(f"RST file created for module: {relative_path}")
...
def process_directory(directory, output_dir, base_dir, ai_client):
    """
    Processes a directory, generating RST files for Python modules and packages.
    """
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for file in files:
            if file.endswith(".py") and file != "__init__.py":
                ...
                create_rst_file(
                    file_path, output_dir, os.path.splitext(file_relative_path)[0], ai_client
                )
...
def setup_sphinx_project(project_name, project_root, output_dir, ai_client):
    """
    Sets up the Sphinx documentation project and generates RST files for modules.
    """
    ...
    for dir_name in ["data", "icons", "images", "lib", "src", "test", "tools"]:
        ...
        if os.path.exists(dir_path):
            process_directory(dir_path, output_dir, project_root, ai_client)
    print(f"Sphinx project has been set up in the '{output_dir}' directory.")


def main():
    """
    Main function to set up the Sphinx project and generate documentation.
    """
    project_name = "ScriptsEditor"
    project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    output_dir = os.path.join(project_root, "docs")

    debug_print(f"Starting setup for project: {project_name}")

    # Initialize the AI client (ensure your client works with localhost:8004)
    client = openai.OpenAI(base_url="http://localhost:8004/v1/", api_key="not-needed")

    # Set up the Sphinx project
    setup_sphinx_project(project_name, project_root, output_dir, client)

    debug_print("Sphinx project setup complete.")
