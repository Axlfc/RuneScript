...
def export_python_scripts_to_txt(directory, output_filename):
    ...
    Args:
        ...
        output_filename (Any): Description of output_filename.
...
def export_all_python_scripts_to_txt_from_mvc_project(
    ...
    Args:
        main_directory_path (Any): Description of main_directory_path.
        output_filename (Any): Description of output_filename.
    ...
    try:
        with open(output_filename, "w", encoding="utf-8") as output_file:
            for section_name, directory in sections.items():
                ...
                if not os.path.isdir(directory):
                    output_file.write(
                        f"The directory for {section_name} does not exist.\n"
                    )
                    continue
                for filename in os.listdir(directory):
                    if filename.endswith(".py"):
