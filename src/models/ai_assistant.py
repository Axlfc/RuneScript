...
def add_message(session_id, message):
    ...
    if not os.path.isfile(file_path):
        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            data = json.load(file)
    data['messages'].append({'timestamp': datetime.now().strftime('%Y-%m-%dT%H:%M:%S'), 'content': message})
...
def find_gguf_file():
    ...
    if len(gguf_files) == 0:
        ...
        raise ValueError(f'Multiple .gguf files found in directory: {MODEL_DIR}. Please ensure there is only one .gguf file.')
...
def chat_loop(prompt, client, model_path, system_prompt='You are an intelligent assistant. You always flawlessly provide straight to the point well-reasoned answers that are both correct and helpful.', session_id=0):
...
def initialize_gemini_client():
    ...
    return {'api_key': GEMINI_API_KEY, 'base_url': 'https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent'}

def process_gemini_chat(client, prompt):
    headers = {'Content-Type': 'application/json'}
    data = {'contents': [{'parts': [{'text': prompt}]}]}
    try:
        response = requests.post(f"{client['base_url']}?key={client['api_key']}", headers=headers, json=data)
...
def process_claude_chat(client, prompt):
    try:
        response = client.completions.create(model='claude-3-opus-20240229', max_tokens_to_sample=1000, temperature=0.7, prompt=f)
