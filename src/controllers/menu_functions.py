...
from src.controllers.scheduled_tasks import open_cron_window, open_at_window, open_scheduled_tasks_window, open_new_at_task_window, open_new_crontab_task_window
from src.models.file_operations import prompt_rename_file
from src.models.script_operations import get_operative_system, see_stdout, see_stderr, run_script, run_script_windows, run_script_with_timeout
from src.localization import localization_data
from src.views.edit_operations import undo, redo, duplicate, copy, cut, paste
from src.views.tk_utils import toolbar, menu, root, script_name_label, script_text, is_modified, file_name, last_saved_content, local_python_var, show_directory_view_var, show_file_view_var, frm, directory_label, script_frm, content_frm, entry_arguments_entry, generate_stdin, generate_stdin_err, show_arguments_view_var, show_run_view_var, run_frm, line_frm, show_timeout_view_var, show_interactive_view_var, interactive_frm, show_filesystem_view_var, filesystem_frm
from src.controllers.tool_functions import find_text, change_color, open_search_replace_dialog, open_terminal_window, open_ai_assistant_window, open_webview, open_terminal_window, create_url_input_window, open_ipynb_window, create_settings_window, open_git_window, open_image_generation_window, open_music_generation_window, open_audio_generation_window, open_kanban_window, open_winget_window, open_system_info_window, open_calculator_window, open_translator_window
...
from src.controllers.file_operations import open_file, open_script, update_title, file_types, new, save, save_script, save_as_new_script, close
...
def about(event=None):
    messagebox.showinfo(localization_data['about_scripts_editor'], localization_data['scripts_editor_info'])
...
def toggle_timeout_view_visibility(frame):
    if show_timeout_view_var.get() == 1:
        ...
        if get_operative_system() != 'Windows':
            ...
            run_button = Button(frame, text=run_icon, command=lambda: run_script_with_timeout(timeout_seconds=float(seconds_entry.get())))
...
def add_view_section_to_menu(options_parameter_name, view_variable, menu_section, view_section_name, frame, function):
    ...
    menu_section.add_checkbutton(label=view_section_name, onvalue=1, offvalue=0, variable=view_variable, command=lambda: toggle_view(frame, function, options_parameter_name, view_variable))
...
def create_menu():
    ...
    file_menu.add_command(label='New', command=new, compound='left', image=image_new, accelerator='Ctrl+N', underline=0)
    file_menu.add_command(label='Open', command=open_script, compound='left', image=image_open, accelerator='Ctrl+O', underline=0)
    file_menu.add_command(label='Close', command=open_script, compound='left', image=None, accelerator='Ctrl+W', underline=0)
    file_menu.add_command(label='Save', command=save_script, compound='left', image=image_save, accelerator='Ctrl+S', underline=0)
    ...
    edit_menu.add_command(label='Paste', command=paste, compound='left', image=image_paste, accelerator='Ctrl+V', underline=0)
    edit_menu.add_separator()
    ...
    add_view_section_to_menu('is_directory_view_visible', show_directory_view_var, view_menu, 'Directory', frm, toggle_directory_view_visibility)
    add_view_section_to_menu('is_file_view_visible', show_file_view_var, view_menu, 'File', script_frm, toggle_file_view_visibility)
    view_menu.add_separator()
    add_view_section_to_menu('is_arguments_view_visible', show_arguments_view_var, view_menu, 'Script Arguments', content_frm, toggle_arguments_view_visibility)
    add_view_section_to_menu('is_run_view_visible', show_run_view_var, view_menu, 'Run', run_frm, toggle_run_view_visibility)
    add_view_section_to_menu('is_timeout_view_visible', show_timeout_view_var, view_menu, 'Timeout', line_frm, toggle_timeout_view_visibility)
    add_view_section_to_menu('is_interactive_view_visible', show_interactive_view_var, view_menu, 'Interactive', interactive_frm, toggle_interactive_view_visibility)
    add_view_section_to_menu('is_filesystem_view_visible', show_filesystem_view_var, view_menu, 'Filesystem', filesystem_frm, toggle_filesystem_view_visibility)
    tool_menu = Menu(menu)
    ...
    tool_menu.add_command(label='Translator', command=open_translator_window, accelerator='Alt+K')
    ...
    tool_menu.add_command(label='Options...', command=create_settings_window, compound='left', accelerator='Alt+S', underline=0)
